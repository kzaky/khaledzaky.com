...
The citation and voice audits sat at passes 5 and 6. Both used Claude Haiku with a `4096` output token budget. That decision made sense when I built it. Haiku is fast, cheap, and more than capable of reading a draft and applying targeted corrections.

The problem wasn't the model. It was the token budget.
...
The citation and voice audits were the wrong shape of task for Haiku at `4096` tokens. The cost optimization made sense in isolation. Applied to the wrong pass, it introduced a failure mode that was invisible until I went looking for it.
...
- **Size models to the task shape, not just the cost.** Haiku for structural operations, Sonnet for full-document rewrites. The token budget needs to fit the output, not just the model tier.
