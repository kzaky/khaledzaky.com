...
![Static API Keys (Antipattern)](/postimages/charts/the-ietf-is-now-working-on-agent-authentication-here-is-what-that-means-diagram-2.svg)

![Agent Delegation Patterns](/postimages/charts/the-ietf-is-now-working-on-agent-authentication-here-is-what-that-means-diagram-2.svg)
...
In ['Agents Are Not Software'](https://khaledzaky.com/blog/agents-are-not-software) I wrote: "We're introducing a new category of participant inside enterprise systems: non-human actors operating with delegated authority. Enterprise infrastructure wasn't originally designed for this."
...
![Non-human identities now outnumber human identities by 45x](/postimages/charts/the-ietf-is-now-working-on-agent-authentication-here-is-what-that-means-chart-1.svg)
*Source: General knowledge. Chart generated by blog agent.*

A few things stand out:

**The antipattern call-out is important.** Static API keys as an antipattern isn't a new idea. The security community has said this about service accounts for years. But having it stated explicitly in an IETF context, in the agent-specific framing, signals that this is the baseline expectation the community is converging on. 

Organizations still wiring agents through shared API keys are already behind where the draft sets the floor. [^1]

**AIMS as a conceptual stack.** The layered model (identifier → credentials → attestation → provisioning → authentication → authorization → observability → policy → compliance) is a useful frame. We've been building toward something like this at RBC Borealis but without a clean vocabulary for it.

The AIMS model gives a shared language. Not because the draft will become an RFC unchanged, but because having a name for the thing helps teams have the right conversations.
...
**The draft is still early.** Section 14 (Security Considerations) and Section 15 (Privacy Considerations) both say 'TODO.' That's honest. This is genuinely work in progress. The cross-domain authorization section acknowledges that not all scenarios are fully covered. The CIBA-for-mid-execution HITL problem is flagged as needing more specification work. The authors aren't overclaiming.

## The pattern I keep seeing

In ['The Conversation After'](https://khaledzaky.com/blog/the-conversation-after-agentic-ai-needs-a-platform-mindset) I wrote: "Agent ecosystems are unlikely to standardize around products. They'll standardize around protocols and primitives."

This draft is evidence of that happening. WIMSE, SPIFFE, OAuth Token Exchange, Transaction Tokens, HTTP Message Signatures, CIBA, Shared Signals — none of these are new. They're existing, deployed, battle-tested primitives. The draft's contribution is the composition: here's how you assemble them specifically for the agent case.
...
That's how internet standards usually work. HTTP existed before REST. OAuth existed before it became the default for API authorization. The primitive arrives first. The integration pattern gets specified second. Adoption follows.
...
If you're building agent systems in a regulated environment:

1. Read the AIMS stack. Even if you don't implement it fully, the layer model helps you audit where your gaps are. If you don't have an answer for 'attestation' or 'credential provisioning,' you've got gaps worth naming.

2. Move away from static credentials for agents now. Not because the draft says so, but because the [security argument is independently correct](https://www.csoonline.com/article/3629417/why-api-keys-are-a-security-risk-and-what-to-do-about-it.html)
<!-- ⚠️ CITATION FAIL: The page could not be fetched (HTTP status 404), and the claim is not directly supported by the blog post. -->. Short-lived, dynamically provisioned credentials via SPIFFE or equivalent is achievable today.

3. Model your delegation chains explicitly. Every time an agent invokes another agent or system, ask: what authority is being delegated, to what scope, for how long, and can it be revoked mid-execution? Transaction tokens are worth understanding even if you don't implement them yet.

4. Treat HITL as an authorization event. If your human approval flow is just a callback with no authorization-server binding, that's a gap. It may be acceptable today. It won't be acceptable in a regulated deployment at scale.

5. Instrument everything. The draft's observability requirements aren't aspirational. Tamper-evident audit logs, correlated across agents, tools, and LLMs. If you can't reconstruct "which agent did what, using which authorization context, and why access changed over time," you're not ready for production at scale.
...
![Non-human identities vs. human identities](/postimages/charts/the-ietf-is-now-working-on-agent-authentication-here-is-what-that-means-chart-1.svg)
*Source: General knowledge. Chart generated by blog agent.*

![Cloud deployments using OpenAI/Azure AI services at 67% adoption](/postimages/charts/the-ietf-is-now-working-on-agent-authentication-here-is-what-that-means-chart-2.svg)
*Source: General knowledge. Chart generated by blog agent.*

![Cloud deployments using OpenAI/Azure AI services](/postimages/charts/the-ietf-is-now-working-on-agent-authentication-here-is-what-that-means-chart-2.svg)
*Source: General knowledge. Chart generated by blog agent.*

![Company adoption of agentic AI pilots growing from 25% in 2025 to 50% by 2027](/postimages/charts/the-ietf-is-now-working-on-agent-authentication-here-is-what-that-means-chart-3.svg)
*Source: General knowledge. Chart generated by blog agent.*

![Company adoption of agentic AI pilots by year](/postimages/charts/the-ietf-is-now-working-on-agent-authentication-here-is-what-that-means-chart-3.svg)
*Source: General knowledge. Chart generated by blog agent.*

![Enterprise applications including agentic AI by 2028](/postimages/charts/the-ietf-is-now-working-on-agent-authentication-here-is-what-that-means-chart-4.svg)
*Source: General knowledge. Chart generated by blog agent.*
