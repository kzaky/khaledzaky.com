...
for fn in research draft verify notify publish approve ingest chart upload; do
  if [ -d "$fn" ]; then
    pushd "$fn" > /dev/null
...
# Cleanup zips
rm -f research.zip draft.zip verify.zip notify.zip publish.zip approve.zip ingest.zip chart.zip upload.zip
