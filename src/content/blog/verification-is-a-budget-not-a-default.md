...
Verifying everything is a good instinct applied without a budget.
...
It is also the most expensive and slowest option on the board, and it is probabilistic. A judge is a second inference on top of the first. You pay for its tokens whether or not it catches anything, and it adds its own latency to every response it inspects. As a directional sense of scale (mid-2026, order of magnitude, not a benchmark), a judge call roughly doubles the token cost of the step it checks and adds anywhere from a few hundred milliseconds to a couple of seconds before the output clears. The digits move with model and length. The shape does not. And there is a deeper problem: an LLM judge can be wrong in the same ways the output it judges can be wrong, because it is the same class of system. Using an LLM to certify an LLM is asking a probabilistic system to vouch for a probabilistic system.
...
The kind of answer matters. A test checks the requests you thought to try. Zelkova checks every possible request, because it reasons over the math instead of sampling the behavior. IAM Access Analyzer, the capability many AWS customers use to find resources exposed outside their zone of trust, is Zelkova underneath, and AWS runs this more than a billion times a day. ([source](https://www.amazon.science/blog/a-billion-smt-queries-a-day)) AWS calls it provable security.
...
There is a reason evaluation feels like a data-science problem, and it is a problem worth fixing. The people who can specify what a correct answer looks like are usually not the people who can build an evaluator. A compliance officer knows the disclosure rule cold. A relationship manager knows what a suitable recommendation is. Neither of them writes prompts or SMT formulas, so "what good looks like" gets translated, lossily, by an engineer who does not own the risk.
